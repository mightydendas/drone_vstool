...
public class WebSocketClient : Singleton<WebSocketClient> {
    ...
    private readonly Dictionary<string, Action<JObject>> unsolicitedHandlers = new Dictionary<string, Action<JObject>>();
    ...
    public void Send(RequestJson request, Action<JObject> onSuccess, Action<string> onError, int timeoutSeconds = 5) {
        request.RequestId = Guid.NewGuid().ToString();
        string json = JsonConvert.SerializeObject(request);

        PendingRequest pendingRequest = new PendingRequest(request.Type, request.RequestId, onSuccess, onError);
